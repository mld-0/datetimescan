...
bin_datetimescan=""		#	set by 'build_release' / 'build_debug'
...
run_tests() {
	...
	if [[ ! -x "$bin_datetimescan" ]]; then
		echoerr "$func_name, failed to find bin_datetimescan=($bin_datetimescan)"
	fi
...
#	{{{
	...
	test_cmd=( $bin_datetimescan locate )
	result_str=$( echo "" | ${test_cmd[@]} )
	expected_str=\
""
	assert_result

	test_cmd=( $bin_datetimescan locate --input "$path_testfile_empty" )
	result_str=$( ${test_cmd[@]} )
	expected_str=\
""
	assert_result
